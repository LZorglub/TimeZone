...
namespace Afk.ZoneInfo
{
    ...
    static class TzUtilities
    {
        ...
        public static TimeSpan GetHMS(string time)
        {
            ...
            int hour = Convert.ToInt32(array[0], CultureInfo.InvariantCulture) * 3600;
            int min = 0;
            int sec = 0;
            ...
            if (array.Length > 1)
            {
                min = Convert.ToInt32(array[1], CultureInfo.InvariantCulture) * ((sign == 0) ? 1 : sign) * 60;
            }

            if (array.Length > 2)
            {
                sec = Convert.ToInt32(array[2], CultureInfo.InvariantCulture) * ((sign == 0) ? 1 : sign);
            }
        ...
        {
            if (string.IsNullOrEmpty(month)) throw new ArgumentNullException(nameof(month));

#pragma warning disable CA1308 // Normaliser les chaînes en majuscules
            string lowerMonth = month.ToLowerInvariant();
...
#pragma warning disable CA1308 // Normaliser les chaînes en majuscules
            string labbr = abbr.ToLowerInvariant();
...
#pragma warning restore CA1308 // Normaliser les chaînes en majuscules
        ...
        public static Rule GetRule(List<string> ruleDescription)
        {
            ...
            if (!by.Any())
            {
                r.LowerYear = Convert.ToInt32(loyearp, CultureInfo.InvariantCulture);
            }
            else if (by.Count() == 1)
            {
            ...
            if (!by.Any())
            {
                r.HighYear = Convert.ToInt32(hiyearp, CultureInfo.InvariantCulture);
            }
            else if (by.Count() == 1)
            {
        ...
        {
            if (dateTimeKind == DateTimeKind.Unspecified) throw new ArgumentException("Invalid datetime kind unspecified", nameof(dateTimeKind));
        ...
        {
            if (dateTimeKind == DateTimeKind.Unspecified) throw new ArgumentException("Invalid datetime kind unspecified", nameof(dateTimeKind));
            if (dateTime.Kind == DateTimeKind.Unspecified) throw new ArgumentException("Invalid datetime kind unspecified", nameof(dateTime));
